  ...
  module Shared
    ...
    module TaggableQuery
      def filter_ids
        ...
        @filter_ids += tag_field_ids
        @filter_ids += parsed_included_tags[:ids]
        @filter_ids = @filter_ids.uniq
      end

      # For rating_ids, fandom_ids, etc. get the ids (if any)
      # from the options and combine them into one array
      def tag_field_ids
        tag_type_id_list.map do |tag_type|
          [options[tag_type]]
        end.flatten.compact
      end
      ...
      def parsed_included_tags
        @parsed_included_tags ||= parse_named_tags(
          %i(fandom_names character_names relationship_names
             freeform_names other_tag_names)
        )
      end
      ...
      def tag_type_id_list
        Tag::TAGGABLE_TYPES.map do |tag_type|
          "#{tag_type.humanize}_ids".to_sym
        end
      end
    end
  end
