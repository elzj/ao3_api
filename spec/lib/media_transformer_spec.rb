...
describe Otw::Sanitizer::Transformers::MediaTransformer do
  let(:transformer) { Otw::Sanitizer::Transformers::MediaTransformer }
  ...
  context "when sanitizing" do
    ...
    context "given a blacklisted source" do
      before do
        ArchiveConfig.media[:blacklisted_srcs] = ["google.com"]
      end

      after do
        ArchiveConfig.media[:blacklisted_srcs] = []
      end
