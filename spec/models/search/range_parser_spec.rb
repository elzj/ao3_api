  ...
  describe '.string_to_range' do
    ...
    context 'when given a date range' do
      it 'returns a max value with greater than' do
        ...
        expect(range).to match(max: date)
      end
      ...
      it 'returns a min value with less than' do
        ...
        range = Search::RangeParser.string_to_range(str)
        expect(range).to match(min: date)
      end
      ...
      it 'returns a range with a dash' do
        ...
        range = Search::RangeParser.string_to_range(str)
        expect(range).to match(min: min_date, max: max_date)
      end
    end
    ...
    context 'when given a number range' do
      it 'returns a min value with greater than' do
        str = ">10"
        range = Search::RangeParser.string_to_range(str)
        expect(range).to match(min: 10)
      end

      it 'returns a max value with less than' do
        str = "< 99"
        range = Search::RangeParser.string_to_range(str)
        expect(range).to match(max: 99)
      end

      it 'returns a range with a dash' do
        str = "1000-8000"
        range = Search::RangeParser.string_to_range(str)
        expect(range).to match(min: 1000, max: 8000)
      end

      it 'ignores whitespace' do
        str = "6  -  66"
        range = Search::RangeParser.string_to_range(str)
        expect(range).to match(min: 6, max: 66)
      end

      it 'ignores commas' do
        str = "1,000 - 10,000"
        range = Search::RangeParser.string_to_range(str)
        expect(range).to match(min: 1000, max: 10000)
      end
    end
  end
end
