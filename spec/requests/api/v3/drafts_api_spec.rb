...
describe "Drafts API", type: :request do
  ...
  describe "#index" do
    ...
    it "returns an array of drafts" do
      get "/api/v3/drafts.json", headers: auth_header
  ...
  describe "#show" do
    ...
    it "returns an array of drafts" do
      get "/api/v3/drafts/#{draft.id}.json", headers: auth_header
  ...
  describe "#create" do
    it "creates a draft" do
      ...
      post "/api/v3/drafts.json", params: { draft: attributes }, headers: auth_header
      expect(response).to have_http_status(:created)
      data = JSON.parse(response.body)
      expect(data.keys).to include("id")

      get "/api/v3/drafts/#{data['id']}.json", headers: auth_header
      data = JSON.parse(response.body)
      expect(data.keys).to include("title")
